...
const PLANES = {
  ...
  runtime: { c: '#A78BFA', label: 'Runtime reuse protocol — current' },
...
const BANDS = [
  ...
  { id: 'band-future-core', plane: null, x: 160, y: 1150, w: 1260, h: 330, stroke: '#3A4250', alpha: 1,
    hdr: { x: 184, y: 1180, t: 'RUNTIME PROTOCOL + FUTURE CORE CAPABILITIES', fill: C.slate },
    tagr: { x: 1396, y: 1180, t: 'CONTRACTS + FUTURE OWNED ABSTRACTIONS', fill: C.slate, alpha: 0.7 } },
...
const BOXES = [
  ...
  { id: 'runtime-registry', plane: 'runtime', band: 'band-future-core', x: 500, y: 1240, w: 270, h: 104, r: 10, dash: true,
    name: 'RuntimeRegistry — internal port', about: '已交付 Registry／Retention contract；與 CacheStore 分離，但不是 backend。',
    texts: [ ['bl', 524, 1268, 'RuntimeRegistry — internal port'], ['bs', 524, 1290, 'lookup / retain contracts'], ['bs', 524, 1306, '與 CacheStore 分離'], ['bn', 524, 1328, 'no backend or mapper'] ] },
...
const TEXTS = [
  ...
  { s: 'sub', x: 160, y: 118, t: 'provider-agnostic library：確認 identity → 安全重用 response → runtime reuse protocol → 未來執行' },
  ...
  { s: 'plane', x: 485, y: 1214, t: 'RUNTIME REUSE PROTOCOL — CURRENT', anchor: 'center', fill: planeColor('runtime') },
  ...
  { s: 'bn', x: 160, y: 1864, t: 'boundary：CacheStore 位於 Response Reuse 內部；RuntimeRegistry 是獨立 protocol，不是 backend' },
  { s: 'bn', x: 160, y: 1884, t: 'boundary：ModelIdentity → RuntimeReuseKey 僅在外部 ACL；Loaded Runtime Cache 不 direct import Identity' }
];
