...
const PLANES = {
  ...
  execution: { c: '#F6821F', label: 'Model execution — coordination contracts implemented' },
...
const BANDS = [
  ...
  { id: 'band-future-core', plane: null, x: 160, y: 1150, w: 1260, h: 330, stroke: '#3A4250', alpha: 1,
    hdr: { x: 184, y: 1180, t: 'RUNTIME + EXECUTION CAPABILITIES — MIXED DELIVERY STATUS', fill: C.slate },
...
const BOXES = [
  ...
  { id: 'reuse-bc', plane: 'reuse', band: 'band-reuse', x: 200, y: 840, w: 500, h: 120, r: 10, dash: true,
    name: 'Response Reuse BC', about: '只消費已確認 identity；依固定 codec 編解碼 response，決定安全重用。',
    texts: [ ['bl', 224, 868, 'Response Reuse Protocol'], ['bs', 224, 890, '只消費 opaque confirmed identity'], ['bs', 224, 906, '編解碼 bytes，不執行模型'], ['bn', 224, 934, 'independent protocol topic'] ] },
  { id: 'reuse-decision', plane: 'reuse', band: 'band-reuse', x: 750, y: 840, w: 270, h: 104, r: 10, dash: true,
    name: 'Lookup outcomes', about: '依已存 codec id 解碼後產生 Hit；未知 codec 或壞 bytes 產生 Unavailable。',
    texts: [ ['bl', 774, 868, 'Lookup outcomes'], ['bs', 774, 890, 'Hit → 解碼原生 response'], ['bs', 774, 906, 'Miss → future handoff'], ['bn', 774, 928, 'Unavailable → boundary stop'] ] },
  { id: 'cache-store', plane: 'reuse', band: 'band-reuse', x: 1070, y: 840, w: 290, h: 104, r: 10, dash: true,
    name: 'CacheStore — internal', about: 'Response Reuse 內部元件，只保存與取回帶 codec id 的 bytes envelope。',
    texts: [ ['bl', 1094, 868, 'CacheStore — internal'], ['bs', 1094, 890, '保存 codec id + bytes'], ['bs', 1094, 906, '不選 codec 或管理 runtime'], ['bn', 1094, 928, '不是頂層 BC'] ] },
  { id: 'reuse-return', plane: 'reuse', band: 'band-reuse', x: 540, y: 980, w: 310, h: 52, r: 10, dash: true,
    name: 'Reused response return', about: '安全重用決策完成後，將解碼後的原生 ModelResponse 回傳給 consumer。',
    texts: [ ['bl', 564, 1008, 'Reused response return'], ['bs', 564, 1024, 'Hit decoded-return path'] ] },
  { id: 'unavailable-stop', plane: 'reuse', band: 'band-reuse', x: 880, y: 980, w: 230, h: 52, r: 10, dash: true,
    name: 'Unavailable boundary stop', about: 'Store read failure、未知 codec 或壞 bytes 停在 Response Reuse boundary，不能轉為 Miss。',
  ...
  { id: 'execution', plane: 'execution', band: 'band-future-core', x: 870, y: 1240, w: 490, h: 104, r: 10, dash: true,
    name: 'Model Execution', about: '已實作 provider-neutral、同步、可注入的 runtime access／invocation ports、outcomes 與 protocol orchestration；實際 wiring 與跨 BC 組裝仍屬未來。',
    texts: [ ['bl', 894, 1268, 'Model Execution'], ['bs', 894, 1290, '同步、可注入 coordination'], ['bs', 894, 1306, 'ports + outcomes + protocol'], ['bn', 894, 1328, 'contracts implemented · integration future'] ] },
...
const EDGES = [
  ...
  { from: 'reuse-bc', to: 'retention-result', pts: [[702,900],[1248,900],[1248,974]], label: { s: 'al', x: 975, y: 888, t: 'record → 編碼 bytes', anchor: 'center' } },
...
const TEXTS = [
  ...
  { s: 'tag', x: 160, y: 146, runs: [
    ...
    { t: 'Loaded Runtime Cache', fill: C.violet }, { t: ' → ', fill: '#4A5462' },
    { t: 'Model Execution', fill: C.orange }, { t: ' → ', fill: '#4A5462' },
    { t: 'Provider Adapter', fill: C.rose } ] },
  ...
  { s: 'plane', x: 1115, y: 1214, t: 'MODEL EXECUTION — CONTRACTS IMPLEMENTED', anchor: 'center', fill: planeColor('execution') },
  ...
  { s: 'bn', x: 160, y: 1884, t: 'status：Model Execution 已有 direct-import executable modules；runtime/provider/cross-BC integration 仍屬未來' }
];
