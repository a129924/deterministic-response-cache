...
2. `ResponseCodec[PayloadT]` 恰定義 `codec_id` property、`encode(ModelResponse[PayloadT]) -> StoredResponse`、`decode(bytes) -> ModelResponse[PayloadT]`。`JsonResponseCodec(ResponseCodec[JsonPayload])` 與 `PyArrowDataFrameCodec(ResponseCodec[pandas.DataFrame])` 明確繼承，三個 member 以 Python 3.12 `typing.override` 實作；property decorator 順序為 `@property`、`@override`。
...
- **Then:** Store 保存 `StoredResponse(codec_id=JSON_V1, payload=<bytes>)`；lookup 只依 id decode，回傳原生 dict 的 `Hit(ModelResponse(...))`，policy 評估解碼後 VO，identity 原樣傳遞。
