...
The currently active route is C0S, a Human-authorized status-only direct child candidate of C2.
It synchronizes the already-committed C1/C2 facts across the four CSO1 planning state surfaces
before C3 begins. This file, its paired correction step, the parent plan, and the parent step
tracker are C0S's only mutable state surfaces. The prior original six-path subject and all CAVO1
records remain frozen provenance: they cannot be reused as a CSO1 candidate, Tester record, or
Reviewer record.
...
## C0S status-synchronization boundary

- C0S changes exactly six planning paths: the parent plan, parent step tracker, this CSO1
  correction plan, this CSO1 correction step, and the paired C0S correction plan/step. It records
  only the frozen C0/C1/C2 facts, marks C1 and C2 complete, leaves C3 pending, and marks C4–C8 as
  not started. It creates no implementation subject or execution evidence.
- C0S must be a non-merge direct child whose sole first parent is C2
  `15c23d67856b627fa8f736eb66694cccc9e5ec89`. Its first-parent name-status diff must contain
...
- No C0S commit SHA, tree SHA, candidate blob SHA, review outcome, or C1S evidence may be filled
  in before C0S is committed. `README.md`, `pyproject.toml`, and `uv.lock` remain unstaged and
  unchanged. C0S never modifies analysis, the parent specification, code, tests, diagrams,
...
- The independent C1S reviewer must fail closed unless all four state surfaces agree on the same
  C0/C1/C2 facts, C1/C2 complete status, C3 as the only pending CSO1 execution step, and C4–C8 as
  not started. An approved C1S log is separately committed unchanged by an Independent Implementer
  as C2S; only then may Planner route the existing C3 subject.
...
| C0 | Parent step tracker | `plan/canonical-identity-pipeline/canonical-identity-pipeline.step.md` | Plan-Creator | CSO1 progression truth, synchronized by C0S only. |
| C0 | Correction plan | `plan/canonical-identity-pipeline/canonical-identity-pipeline.concrete-stage-override.correction-plan.md` | Plan-Creator | This CSO1 trigger, scope, ordering, and schema authority; synchronized by C0S only. |
| C0 | Correction step | `plan/canonical-identity-pipeline/canonical-identity-pipeline.concrete-stage-override.correction-step.md` | Plan-Creator | CSO1 execution/evidence tracking; synchronized by C0S only. |
...
| C0S | Status-sync candidate | parent plan, parent step, this CSO1 plan/step, and paired C0S plan/step | Plan-Creator | Exact six-path non-merge direct child of C2; records state only and awaits C1S. |
| C1S | Status-sync Plan-Reviewer log | `plan/canonical-identity-pipeline/canonical-identity-pipeline.concrete-stage-override-status-sync.correction-plan-review-log.json` | Independent Plan-Reviewer | Sole C0S review artifact, written only from a clean committed C0S checkout under the schema below. |
...
Only the independent C1S Plan-Reviewer may create this file, only after the C0S candidate is
...
altered nested shape, mismatched C2 parent, merge topology, unlisted path, non-clean review,
inconsistent four-surface facts, a C3 completion claim, or a publish/PR/Human-review claim fails
closed.
...
The reviewer's C1S file is not prewritten, is not part of C0S, and cannot be combined with any
other path. Only an Independent Implementer may commit an unchanged approved C1S file as C2S.
After C2S, Planner must independently perform the route decision; C0S never itself selects C3.
