...
> **B4R3 current execution truth:** `0800dc11181cdbd7d93d85e0298ea78dc33d06d3` committed
> B4R2, but its clean-checkout planning review failed. B0, B1, B2, B3, B4, B4R, B4R2,
...
> deferred. B4R3 is the sole current pre-subject route: clean review of committed B4R3, then S5,
> T5 and V5 only.

## Goal / Outcome

完成 current-topic governance correction：以 B4R3 的獨立 clean-checkout planning review 建立唯一
pre-subject gate，之後只允許 immutable S5 與 T5/V5 evidence chain，並保持 Human boundary。

## Scope

- **In scope:** B4R3 的七個 planning paths；approved B4R3 review 後，S5 的 exact 15-path
  allowlist；以及 B4R3 exact T5/V5 evidence paths。
...
- `0800dc11181cdbd7d93d85e0298ea78dc33d06d3` B4R2 and its failed clean review are frozen
  non-subject provenance; B4R3 and its separately committed review record are also non-subject;
  only S5 can establish `implementation_subject_sha`.
...
Reviewer independently write only declared evidence. Frozen B2/B3/B4/B4R/B4R2 records have no
...
`B4R3 review -> S5 -> T5 -> V5 -> human-check`. Any contract failure returns to Planner; no prior
epoch is a fallback.
...
| --- | --- | --- | --- | --- |
| Shared workflow | `plan/agent-handoff-workflow.md` | Plan-Creator | Planner | B4R3 contract |
| Shared contract | `plan/topic-plan-contract.md` | Plan-Creator | Planner | B4R3 contract |
...
| B4R3 plan | `plan/observer-dispatcher-governance/observer-dispatcher-governance.correction-b4r3-plan.md` | Plan-Creator | Planner | B4R3 delta |
| B4R3 step | `plan/observer-dispatcher-governance/observer-dispatcher-governance.correction-b4r3-step.md` | Plan-Creator | Planner | B4R3 tracker |
| B4R3 review | `plan/observer-dispatcher-governance/observer-dispatcher-governance.correction-b4r3-review-log.md` | Plan-Reviewer | Plan-Reviewer verdict; Planner route | Pre-S5 gate |
| T5 evidence | `plan/observer-dispatcher-governance/observer-dispatcher-governance.correction-b4r3-tester-evidence.md` | Tester | Factual test result only | First descendant |
| V5 evidence | `plan/observer-dispatcher-governance/observer-dispatcher-governance.correction-b4r3-implementation-review-log.md` | Reviewer | Reviewer verdict; Planner route | Final descendant |

After separately committed approved B4R3 review, S5 may modify exactly:
...
1. Independently review and separately commit B4R3 planning evidence; B4R3 remains non-subject.
...
- B4R3 review covers exactly its seven committed paths and is approved before S5.
...
- Actual Git commands prove non-merge `S5 -> T5 -> V5` and `git diff --name-status S5..V5` lists
  exactly the two B4R3 evidence paths.
...
```json
{"current_route":"B4R3-review->S5->T5->V5","b4r3_review_path":"plan/observer-dispatcher-governance/observer-dispatcher-governance.correction-b4r3-review-log.md","implementation_subject":"S5 only","range":"S5..V5","verdict":"approved|needs-rework"}
```

The B4R3 review schema and T5/V5 evidence schemas are defined by
`plan/topic-plan-contract.md#b4r3-correction-evidence-schemas`.
...
Only the current B4R3 review and successor S5/T5/V5 gates remain; frozen history creates none.
