...
## B6R7 Acceptance Criteria

1. `B6R7 -> R17 -> S14 -> T14 -> V14 -> Q14` is the sole current route; all earlier routes are frozen
   nonrouting provenance.
2. B6R7 is a non-subject, non-merge first-parent exact-seven planning baseline. R17 independently records the
   committed revision, reviewed tree, all seven declared path/blob pairs, first-parent admission result, Copilot
   feedback triage, verdict and blockers. B6R7/R17 never establish `implementation_subject_sha`.
...
5. S14 tests read the five canonical planning docs plus B6R7 plan/step, enforce temporal frozen provenance, reject
   B6R7/R17/S1–S13 as a current subject, and require only `S14 -> T14 -> V14`.
6. The exact named `S14..V14` range contains only the B6R7 T14/V14 evidence paths. V14 requires passing same-S14
...
The contract fails closed if a prior route is current, B6R7 admission is merge/not-exact-seven, R17 lacks any
required tree/blob/admission/triage field, B6R7/R17 is treated as subject, S14 widens paths or replaces direct imports,
...
action, architecture work, or unlisted implementation path belongs to B6R7.
